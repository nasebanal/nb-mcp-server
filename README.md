...
**Known limitation**: no silent Auth0 token refresh yet (`tokenExchangeCallback` isn't wired up). A long-lived Claude Desktop session will eventually need to reconnect once the underlying Auth0 access token expires. The refresh token is already captured in `props` for when this gets wired up.
